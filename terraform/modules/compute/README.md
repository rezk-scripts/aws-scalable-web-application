...
The Compute layer hosts the application workload and provides the scalability and availability required to serve client requests. It combines load balancing, automated scaling, and reusable instance configuration to ensure the application remains responsive under varying traffic conditions.

![Compute Diagram](../../documentation/screenshots/compute-architecture.png)

## Architecture

Incoming application requests are distributed across EC2 instances by an Application Load Balancer. Instances are created from a Launch Template and managed by an Auto Scaling Group, allowing the application tier to automatically adjust capacity while maintaining high availability across multiple Availability Zones.
...
| [`../networking/README.md`](../networking/README.md) | Networking layer supporting the compute resources. |
